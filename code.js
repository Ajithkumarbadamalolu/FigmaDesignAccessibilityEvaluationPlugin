...
function analyzeAllFrames(detailed_1) {
    return __awaiter(this, arguments, void 0, function* (detailed, selected = false, criteria = 'all') {
        var _a;
        let frames = [];
        if (selected) {
            const selectedNodes = figma.currentPage.selection;
            frames = selectedNodes.filter(node => node.type === 'FRAME' || node.type === 'TEXT');
            if (frames.length === 0) {
            ...
            frames = figma.currentPage.children.filter(node => node.type === 'FRAME' || node.type === 'TEXT');
            if (frames.length === 0) {
        ...
        for (const frame of frames) {
            ...
            yield new Promise(resolve => setTimeout(resolve, 500));
            const bytes = yield frame.exportAsync({ format: 'PNG', constraint: { type: 'SCALE', value: 2 } }); // Exporting at 2x scale for better detail
            ...
            try {
                ...
                const analysisResult = (_a = data.choices[0].message.content) !== null && _a !== void 0 ? _a : "No results available";
                ...
                yield postCommentToFigma(frame.id, analysisResult);
...
function postCommentToFigma(frameId, commentText) {
